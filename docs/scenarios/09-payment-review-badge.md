...
> Status: Partial / verification pending
...
- [~] REV-001 경기 후 리뷰 작성과 반영 (Task 89 API/UI implemented; final backend/frontend validation and route smoke pending)
...
- [ ] 종료된 개인 매치에서 참가자 리뷰 대상을 확인한다.
- [ ] 종료된 팀매치에서 현재 owner/manager가 상대 팀 리뷰 대상을 확인한다.
- [ ] 별점 `1-5`와 하나 이상의 predefined tag로 리뷰를 제출한다.
- [ ] written tab과 received page를 확인한다.
- [ ] 동일 reviewer/target/source 중복 제출을 확인한다.

### Expected

- [ ] 리뷰가 저장된다.
- [ ] 선택한 tag가 `v1_post_event_review_tags`에 저장된다.
- [ ] 중복 제출은 기존 리뷰와 `alreadySubmitted: true`로 수렴하고 duplicate row를 만들지 않는다.
- [ ] 개인 매치 리뷰는 대상 사용자 reputation summary를 실제 리뷰 기반으로 갱신한다.
- [ ] 팀매치 리뷰는 대상 팀 trust score를 실제 리뷰 기반으로 갱신한다.
