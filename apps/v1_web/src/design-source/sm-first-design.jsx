...
/* Source: docs\reference\handoff-sm-new-direction\sports-platform\project\lib\tokens.jsx */
...
Object.assign(globalThis, { TokensCSS, SBtn, Chip, Badge, Progress, Card, Phone, TabBar, BottomNav, GLOBAL_BOTTOM_TABS, AppBar, TopNav, Icon, StatusBar });


/* Source: docs\reference\handoff-sm-new-direction\sports-platform\project\lib\data.jsx */
...
Object.assign(globalThis, { IMG, SPORTS, MATCHES, TEAM_MATCHES, TEAMS, LESSONS, LISTINGS, VENUES, REGIONS, CHATS, NOTIFS });


/* Source: docs\reference\handoff-sm-new-direction\sports-platform\project\lib\signatures.jsx */
...
/* Source: docs\reference\handoff-sm-new-direction\sports-platform\project\lib\screens-sm-revision.jsx */
...
const SMRevisionChatListMobile = () => (
  <SMRevisionShell title="채팅" back search notificationNew bottom navActive="my">
    <div style={{ padding: 20 }}>
      ...
      <button className="tm-btn tm-btn-primary" aria-label="open chat" style={{ position: 'absolute', right: 20, bottom: 90, width: 54, height: 54, borderRadius: 999, padding: 0, display: 'grid', placeItems: 'center', boxShadow: '0 10px 24px rgba(49,130,246,.28)' }}>
...
const SMRevisionPostEventReviewRules = () => (
  <SMRevisionRuleBoard title="14-1 리뷰 · 일정 종료 후 상호 평가" items={[
    { title: '14 포함 범위', body: '14의 리뷰 대기 목록, 대상 선택, 별점, 선택형 태그 1개, 완료 상태, 예외 규칙을 14-1 수정안 안에 모두 포함한다.' },
...
const SMCoreShellPreviewBar = ({ mode }) => {
  ...
  if (mode === 'list') {
    return (
      <div style={{ minHeight: 56, padding: '8px 10px 8px 20px', borderBottom: '1px solid var(--grey100)', display: 'flex', alignItems: 'center', gap: 8 }}>
...
const SMCoreTopBackCompactOptionsMobile = () => (
  <div style={{ width: 375, height: 812, background: 'var(--bg)', fontFamily: 'var(--font)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
    ...
    <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '12px 18px 28px', display: 'grid', gap: 8, alignContent: 'start', WebkitOverflowScrolling: 'touch' }}>
      ...
      <SMCoreShellRuleCard index={2} title="목록 상단바" body="매치, 팀매치, 팀 목록에서는 검색창과 필터를 바로 노출하고 뒤로가기는 두지 않는다. 검색 아이콘은 입력창 내부 오른쪽에 둔다." meta="적용: 매치 목록, 팀매치 목록, 팀 둘러보기" mode="list"/>
...
const SM_CORE_SEARCH_ROWS = {
  ...
  filter: [
    ['default', '검색 + 필터 기본', '목록 상단에서 검색창과 필터 버튼을 같이 두고 뒤로가기는 제외한다.', 'open filter'],
  ...
  return (
    <div style={{ width: 375, height: 812, background: 'var(--bg)', fontFamily: 'var(--font)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '18px 20px 12px', borderBottom: '1px solid var(--grey100)', flexShrink: 0 }}>
        ...
        <div className="tm-text-body" style={{ marginTop: 8 }}>{isBack ? '뒤로가기는 입력창 바깥 왼쪽, 돋보기는 입력창 내부 오른쪽에 둔다.' : '목록형 검색바는 뒤로가기를 제거하고 필터만 입력창 바깥 오른쪽에 둔다.'}</div>
      </div>
...
const SM_FINAL_CORE_MODULES = {
  ...
  '01': { no: '01', name: 'Auth', basis: '01 SM 최종본', component: null },
  '02': { no: '02', name: 'Home', basis: '02 SM5 검색 기준', component: null },
};
...
const SMRevisionMatchSM7TopSearchRules = () => (
  <SMRevisionPlusBoard eyebrow="03 MATCH SM7 · TOP SEARCH" title="00 최종본 목록 상단바 검색 적용 기준" columns={4}>
    {[
      ['상단바 검색', '00 최종본의 목록 상단바처럼 뒤로가기는 제거하고 검색 필드, 검색 실행, 필터를 56px 상단바 안에 둔다.', 'adopt'],
...
const SMRevisionTeamBrowseSearchFilterRulesSM5 = () => (
  <SMRevisionPlusBoard eyebrow="05 TEAM BROWSE SM5 - SEARCH/FILTER" title="00 + 03/04 search/filter 적용 기준" columns={4}>
    {[
      ['00 list topbar', '05 목록은 00 최종본의 목록 상단바 기준으로 뒤로가기 없이 검색바와 필터 버튼을 상단에 유지한다.', 'topbar', 'blue'],
...
const SMRevisionHomeSM4ActionMatrixExport = SMRevisionHomeSM3ActionMatrix;
const SMRevisionHomeSM4StateMatrixExport = SMRevisionHomeSM3StateMatrix;
const SMRevisionHomeSM4FlowExport = SMRevisionHomeSM3Flow;
const SMRevisionMatchParticipantsMobileSM5Export = SMRevisionMatchDetailMobileSM3;

export { TokensCSS, SMCoreTopBackCompactOptionsMobile, SMCoreFixedBottomNavMobile, SMCoreNotificationMobile, SMCoreSearchTypeExceptionMobile, SMRevisionHomeMobileV2, SMRevisionHomeSearchFinalMobile, SMRevisionHomeSearchFinalNoInputMobile, SMRevisionHomeSearchFinalStateMobile, SMRevisionHomeStatesV2, SMRevisionHomeUIRulesV2, SMRevisionHomeSearchFinalRules, SMRevisionHomeSM4ActionMatrixExport as SMRevisionHomeSM4ActionMatrix, SMRevisionHomeFinalSearchActionMatrix, SMRevisionHomeSM4StateMatrixExport as SMRevisionHomeSM4StateMatrix, SMRevisionHomeSM4FlowExport as SMRevisionHomeSM4Flow, SMRevisionHomeNoticeListFinal, SMRevisionHomeNoticeDetailFinal, SMRevisionMatchListMobileSM7, SMRevisionMatchSM7FilterSheetOption, SMRevisionMatchSM7EmptyTextState, SMRevisionMatchSM7SearchErrorToastState, SMRevisionMatchDetailMobileSM3, SMRevisionMatchPaymentCompleteToastSMFinal, SMRevisionMatchJoinFeedbackOverlaySM3, SMRevisionMatchParticipantsMobileSM5Export as SMRevisionMatchParticipantsMobileSM5, SMRevisionMatchCreateListEntrySMFinal, SMRevisionMatchCreateSportStepSMFinal, SMRevisionMatchCreateInfoStepSMFinal, SMRevisionMatchCreatePlaceTimeStepSMFinal, SMRevisionMatchCreateConfirmStepSMFinal, SMRevisionMatchCreateShareCompleteSMFinal, SMRevisionMatchEditSMFinal, SMRevisionTeamMatchListMobileSM4, SMRevisionTeamMatchSM4FilterSheetOption, SMRevisionTeamMatchSM4EmptyTextState, SMRevisionTeamMatchSM4SearchErrorToastState, SMRevisionTeamMatchDetailMobileSM2, SMRevisionTeamMatchCreateListEntrySMFinal, SMRevisionTeamMatchCreateTeamStepSMFinal, SMRevisionTeamMatchCreateSportStepSMFinal, SMRevisionTeamMatchCreateInfoStepSMFinal, SMRevisionTeamMatchCreateConditionStepSMFinal, SMRevisionTeamMatchCreatePlaceTimeStepSMFinal, SMRevisionTeamMatchCreateConfirmStepSMFinal, SMRevisionTeamMatchCreateShareCompleteSMFinal, SMRevisionTeamMatchEditSMFinal, SMRevisionTeamBrowseMobileSM5, SMRevisionTeamBrowseSearchMobileSM5, SMRevisionTeamBrowseFilterSheetSM5, SMRevisionTeamBrowseDetailSM5, SMRevisionTeamBrowseSearchFilterRulesSM5, SMRevisionMyPageSM1, SMRevisionMyMatchesJoinedSM1, SMRevisionMyMatchesCreatedSM1, SMRevisionMyCreatedMatchManageSM1, SMRevisionMyMatchEditSM1, SMRevisionApplicantUserProfileSM1, SMRevisionApplicantTeamProfileSM1, SMRevisionMyTeamMatchesSM1, SMRevisionMyTeamMatchEditSM1, SMRevisionMyTeamsSM1, SMRevisionMyTeamDetailSM1, SMRevisionMyTeamMembersSM1, SMRevisionChatListMobileSM2, SMRevisionChatRoomMobileSM2, SMRevisionNotificationsMobileSM2, SMRevisionAuthSM3Login, SMRevisionAuthEmailLoginSM3, SMRevisionAuthSignupSM3, SMRevisionAuthSM3TermsStep, SMRevisionAuthSM5TermsBeforeSignup, SMRevisionAuthSM5SignupCompleteGuide, SMRevisionAuthSM5SportStep, SMRevisionAuthSM5LevelStep, SMRevisionAuthSM5RegionStep, SMRevisionAuthSM4Confirm, SMRevisionAuthSM4Resume, SMRevisionAuthFinalWelcome, SMRevisionAuthSM6ProviderDenied, SMRevisionAuthSM6MissingEmail, SMRevisionAuthSM6BlockedAccount, SMRevisionAuthSM6AccountResolve, SMRevisionAuthSM6LocationPermission, SMRevisionProfileReviewMobileSM2, SMRevisionProfileStateMobileSM2, SMRevisionPostEventReviewInboxMobile, SMRevisionPostEventReviewSelectMobile, SMRevisionPostEventReviewCompleteMobile, SMRevisionPostEventReviewRules, SMRevisionPostEventReviewInboxMobile14_1, SMRevisionPostEventReviewSelectMobile14_1, SMRevisionPostEventReviewCompleteMobile14_1, SMRevisionPostEventReviewReceivedMobile14_1, SMRevisionPostEventReviewRules14_1, SMRevisionPostEventReviewInboxMobile14_2, SMRevisionPostEventReviewSelectMobile14_2, SMRevisionPostEventReviewCompleteMobile14_2, SMRevisionPostEventReviewReceivedMobile14_2, SMRevisionPaymentMobile, SMRevisionLandingMobile, SMRevisionAdminMobile };
