...
function MyMatchCard({ match, manage }: { match: MyMatch; manage?: boolean }) {
  ...
  const reviewHref = match.reviewHref ?? '/my/reviews';
  return (
    <Card pad={16}>
      ...
      <div className="tm-my-card-actions">
        ...
        {manage ? <button className="tm-btn tm-btn-sm tm-btn-neutral" type="button" disabled>관리 불가</button> : canReview ? <Link className="tm-btn tm-btn-sm tm-btn-primary" href={reviewHref}>리뷰</Link> : <button className="tm-btn tm-btn-sm tm-btn-neutral" type="button" disabled>리뷰 대기</button>}
      </div>
    </Card>
  );
}
